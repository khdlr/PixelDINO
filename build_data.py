...
parser.add_argument("--mode", default='planet',
                    choices=['planet', 's1', 's2', 's2_4w', 's2_ts',
                             'qinghai', 's2_ts_inference'],
...
def load_annotations(shapefile_root, merge_overlapping=False):
    ...
    for date, date_scenes in scenes.groupby('image_date'):
      ...
      for geom in geoms:
        ...
        image_id = region + '_' + date_scenes.iloc[0].image_id + '_merged'
        scene = date_scenes.iloc[:1].copy()
