...
class TileLoader:
  def __init__(self):
    self.collection = pystac_client.Client.open(
        "https://planetarycomputer.microsoft.com/api/stac/v1/",
        modifier=planetary_computer.sign_inplace,
    ).get_collection("sentinel-2-l2a")


  def load_band(self, band_url):



  def load_tile(self, tile):
    """Load a Sentinel-2 tile to a numpy array"""
    item = self.collection.get(tile)
    year = int(acquisition_date[:4])
...
def main():
  ...
  config.update(munchify(yaml.safe_load((args.model_dir / 'config.yml').open())))
  ...
  model = S.apply
  state = load_state(args.model_dir / 'latest.pkl')

  pipeline = pl.map(str.rstrip, sys.stdin)
  pipeline = pl.map(load_tile, pipeline, workers=4)
